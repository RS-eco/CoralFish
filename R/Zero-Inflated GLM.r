...
I=is.na(ParasiteCod$Intensity) | is.na(ParasiteCod$fArea) | is.na(ParasiteCod$fYear) | is.na(ParasiteCod$Length)
...
f1 <- formula(Intensity ~ fArea*fYear + Length | fArea*fYear + Length)           
Nb1 <- zeroinfl(f1, dist = "negbin", link = "logit", data = ParasiteCod2)
summary(Nb1)
...
#Code for ZIP and hurdle, and comparisons
...
# Prepare bootstrap selections and weigths (Harrell FE. Design library. http://lib.stat.cmu.edu/S/Harrell/)
