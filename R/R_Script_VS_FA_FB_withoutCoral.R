...
kw_FA_data<-kruskal.test(AllSpeciesFA~Area, data=data)
kw_FA_data$star <- "" 
...
summarySpeciesFA <- summarySE(data, measurevar="AllSpeciesFA", groupvars="Area")

plotSpeciesFA <- ggplot(summarySpeciesFA, aes(x=Area, y=AllSpeciesFA)) +
...
#Kruskall-Wallis Test, non-parametric rank test
kw_FB_data<-kruskal.test(AllSpeciesFB~Area, data=data)
kw_FB_data$star <- "" 
kw_FB_data$star[kw_FB_data$p.value <= .05]  <- "*" 
kw_FB_data$star[kw_FB_data$p.value <= .01]  <- "**" 
kw_FB_data$star[kw_FB_data$p.value <= .001] <- "***"

#Plot AllSpeciesFB per Area
summarySpeciesFB <- summarySE(data, measurevar="AllSpeciesFB", groupvars="Area")

plotSpeciesFB <- ggplot(summarySpeciesFB, aes(x=Area, y=AllSpeciesFB)) +
  geom_bar(position=position_dodge(.9), colour="black", fill="#FFFFFF", width=.5) +
...
print(plotSpeciesFA, vp = vplayout(1, 1))
print(plotSpeciesFB, vp = vplayout(1, 2))
